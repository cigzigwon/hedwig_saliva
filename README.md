# Hedwig Saliva Adapter

> A Saliva Adapter for [Hedwig](https://github.com/hedwig-im/hedwig)

[![Build Status](https://www.travis-ci.org/hedwig-im/hedwig_saliva.svg?branch=master)](https://www.travis-ci.org/hedwig-im/hedwig_saliva)
...
Add `hedwig_saliva` to your list of dependencies in `mix.exs`:
...
Ensure `hedwig_saliva` is started before your application:

```elixir
def application do
  [extra_applications: [:hedwig_saliva]]
end
```
...
config :alfred, Alfred.Robot,
  adapter: Hedwig.Adapters.Saliva,
  name: "alfred",
  aka: "/",
  responders: [
...
the module responsible for handling all of the Saliva details like connecting and
...
We'll need to provide a few more things in order for us to connect to our Saliva
...
config :alfred, Alfred.Robot,
  ...
  # for now, you can invite your bot to a channel in saliva and it will join
  # automatically
  rooms: [],
  responders: [
    {Hedwig.Responders.Help, []},
...
Saliva and join the rooms it's in based on its Saliva integration. From there, we
can chat with our bot in any Saliva client.
...
Hedwig Saliva source code is licensed under the [MIT License](https://github.com/hedwig-im/hedwig_saliva/blob/master/LICENSE.md).
